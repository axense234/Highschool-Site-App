...
// SCSS
import searchbarStyles from "../scss/components/Searchbar.module.scss";
// Data
import { categoriiAnunturi, individualPagesData, pagesData } from "@/data";
// Redux
...
const Searchbar: FC<SearchbarProps> = ({ setShowSearchbar, showSearchbar }) => {
  ...
  const recommendationsFromTeachers = teachers.map((teacher) => {
    return {
      id: teacher.id,
      dest: `/profesori`,
      label: `Profesor de ${teacher.profesorDe} - ${teacher.username}`,
      type: "teacher",
    };
  });
  ...
  const handleOnRecommendationClick = (rec: PageData) => {
    if (categoryNames.find((cat) => cat === rec.label.split(" ")[1])) {
      ...
      dispatch(addCategoryToggle(rec.label.split(" ")[1] as CategorieAnunt));
    }
